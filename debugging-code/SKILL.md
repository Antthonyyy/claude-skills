...
описание_ru: Интерактивный отладчик — ставит брейкпоинты, идёт по коду по шагам, смотрит реальные значения переменных и стек вызовов, чтобы дойти до корневой причины бага.
когда_использовать_ru: Когда программа падает или выдаёт неверный результат, а отладки через print/console.log уже не хватает и нужно посмотреть состояние вживую.
