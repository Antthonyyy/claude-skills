...
описание_ru: Парсит лидов из LinkedIn Sales Navigator через Vayne и обогащает их верифицированными email-ами через AnyMailFinder — на выходе готовый список для аутрича.
когда_использовать_ru: Когда нужно собрать базу B2B-лидов под холодную рассылку, найти ЛПР по критериям или сделать ресёрч под продажи.
...
```bash
python3 "C:/Users/gnilo/Desktop/Free Claude Code Skills/execution/scrape_linkedin.py" \
...
```bash
python3 "C:/Users/gnilo/Desktop/Free Claude Code Skills/execution/scrape_linkedin.py" --url "YOUR_URL" --check-only
```
Show the user: prospect count, confirm filters look right.

### Step 2: Scrape + Download
```bash
python3 "C:/Users/gnilo/Desktop/Free Claude Code Skills/execution/scrape_linkedin.py" --url "YOUR_URL" --name "My Campaign" --limit 50 --skip-enrichment --skip-sheets
```
...
```bash
python3 "C:/Users/gnilo/Desktop/Free Claude Code Skills/execution/enrich_leads.py" --input .tmp/YOUR_RAW_CSV.csv
```
...
```bash
python3 "C:/Users/gnilo/Desktop/Free Claude Code Skills/execution/export_leads_to_sheets.py" --input .tmp/YOUR_ENRICHED_CSV.csv --create "LinkedIn Leads - Campaign Name" --valid-only
```
