...
описание_ru: Превращает транскрипт (подкаст, выступление, видео) в Twitter-тред, пост для LinkedIn и черновик рассылки — всё параллельно.
когда_использовать_ru: Когда есть один длинный материал (видео/подкаст/встреча) и нужно нарезать из него контент под разные соцсети и email.
...
- **Author name** (optional): Defaults to "Anton" if not specified.
