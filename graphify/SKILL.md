...
```bash
python3.13 -c "import graphify" 2>/dev/null || pip install graphifyy -q --break-system-packages 2>&1 | tail -3
```
...
```bash
python3.13 -m graphify.serve graphify-out/graph.json
```
...
```bash
python3.13 -m graphify.watch INPUT_PATH --debounce 3
```
