...
описание_ru: Полный ревью кода — ищет баги, уязвимости (SQL-инъекции, XSS, небезопасная десериализация), code smells, проблемы с производительностью (N+1) и архитектурой, выдаёт приоритезированный отчёт.
когда_использовать_ru: Когда нужно проверить пулл-реквест, сделать аудит качества кода, найти что отрефакторить или поискать уязвимости перед мержем.
license: MIT
