...
описание_ru: Создание холодных email-кампаний в сервисе Instantly — на основе описания клиента и его офферов собирает готовую цепочку писем для аутрича.
когда_использовать_ru: Когда нужно запустить B2B-аутрич, собрать холодную email-цепочку или автоматизировать рассылку для привлечения лидов.
...
### Step 4: Run
```bash
python3 "C:/Users/gnilo/Desktop/Free Claude Code Skills/execution/instantly_create_campaigns.py" \
...
3. Confirm `INSTANTLY_API_KEY` and `ANTHROPIC_API_KEY` are in `.env`, and the Instantly account has at least 3 existing campaigns to use as source material.
